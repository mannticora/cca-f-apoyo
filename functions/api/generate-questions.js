...
export async function onRequestPost(context) {
  ...
  try {
    ...
    return json(502, { error: "No se pudo contactar la API de Anthropic: " + e.message });
  }

  if (!anthropicRes.ok) {
    let detail = "";
    try { detail = (await anthropicRes.json()).error?.message || ""; } catch (e) {}
    return json(anthropicRes.status, { error: "Anthropic API error (" + anthropicRes.status + "): " + detail });
  }
  ...
  return json(200, { questions, truncated });
}
